...
function CourseDetails() {
  const { user } = useSelector((state) => state.profile);
  const { token } = useSelector((state) => state.auth);
  // normalize token so string "null" or empty is treated as no-token
  const rawToken = token && token !== "null" ? token : null;
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      try {
        console.log(
          "Fetching course details for id:",
          courseId,
          "tokenPresent:",
          Boolean(rawToken)
        );
        ...
        if (status === 401 || /token|login|invalid/i.test(String(serverMsg))) {
          // invalid / expired token — clear it from client and inform user
          toast.error(serverMsg || "Session expired. Please log in again.");
          dispatch(setToken(null));
        } else {
          toast.error(serverMsg || "Could not load course details.");
    ...
    fetch();
  }, [courseId, rawToken, dispatch]);

  // --- aggregated rating & review count from the API ---
  const [avgRating, setAvgRating] = useState(0);
  const [reviewCount, setReviewCount] = useState(0);
    ...
    <>
        ...
        <div className="course-details-container">
          <div className="course-details-main">
            ...
            <div className="course-left-column">
              <div className="course-thumbnail-mobile">
                <div className="thumbnail-shadow"></div>
                <img src={thumbnail} alt="course thumbnail" className="thumbnail-image" />
              </div>
              <div className="course-meta">
                <p className="course-title">{courseName}</p>
                <p className="course-description">{courseDescription}</p>
      ...
      <div className="course-content-wrapper">
        <div className="course-what-you-learn">
          <p className="section-title">What you'll learn</p>
          <div className="section-body">
            <ReactMarkdown remarkPlugins={[remarkGfm]}>{whatYouWillLearn}</ReactMarkdown>
          </div>
        </div>
        ...
        <div className="course-content">
          ...
          <div className="accordion-section">
            {courseContent?.map((course, index) => (
              <div key={index} className="accordion-item">
                <CourseAccordionBar course={course} isActive={isActive} handleActive={handleActive} />
                {isActive.includes(course._id) && (
                  <div className="lecture-list">
                    {course.subSection?.map((lecture, idx) => (
                      <div className="lecture-item" key={idx}>
